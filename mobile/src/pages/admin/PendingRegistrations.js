...
const RegistrationCard = ({
  ...
  const flipToBack = async (doc) => {
    ...
    try {
      if (doc.data && doc.data.startsWith('data:')) {
        ...
        try {
          rawB64 = decodeURIComponent(rawB64);
        } catch (e) {}
      ...
      Alert.alert(t("Preview Error"), err.message || String(err));
      setFlippedDoc(doc);
    }
  ...
  const handleDownload = async (base64, label) => {
    ...
    try {
      ...
      console.error(err);
      Alert.alert(t("Download Error"), err.message || String(err));
    }
  };
  const DocumentRow = ({
    docBase64,
  ...
  return <View style={styles.cardContainer}>
      ...
      <Animated.View pointerEvents={flippedDoc ? 'none' : 'auto'} style={[styles.cardFace, styles.cardFront, frontAnimatedStyle]}>
        ...
        <View style={styles.cardBody}>
          <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Email')}</Text><Text style={styles.infoVal} numberOfLines={1}>{user.Email}</Text></View>
          <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Phone')}</Text><Text style={styles.infoVal} numberOfLines={1}>{user.Phone || 'N/A'}</Text></View>
          <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('GSTIN')}</Text><Text style={styles.infoVal} numberOfLines={1}>{user.BPID || 'N/A'}</Text></View>
        </View>

        <ScrollView style={styles.docsArea} nestedScrollEnabled={true}>
          <View style={styles.docsTitleRow}>
            <FileText size={16} color="#1A1A1A" />
            <Text style={styles.docsTitle}>{t('Attached Documents')}</Text>
          </View>

          {userDocs ? <View>
              <DocumentRow docBase64={userDocs.aadhar} label={t("Aadhar Card")} />
              <DocumentRow docBase64={userDocs.pan} label={t("PAN Card")} />
              <DocumentRow docBase64={userDocs.gst} label={t("GST Certificate")} />
              <DocumentRow docBase64={userDocs.companyPan} label={t("Company PAN")} />
              <DocumentRow docBase64={userDocs.bankCheque} label={t("Bank Cheque")} />
              {!Object.values(userDocs).some(Boolean) && <Text style={styles.noDocsText}>{t('No documents uploaded.')}</Text>}
            </View> : isDocLoading ? <ActivityIndicator color="#2563EB" style={{
          marginVertical: 10
        }} /> : <TouchableOpacity onPress={onLoadDocs} style={styles.loadDocsBtn}>
              <Text style={styles.loadDocsText}>{t('Load Documents')}</Text>
            </TouchableOpacity>}
        </ScrollView>

        <View style={styles.cardActions}>
          <TouchableOpacity style={[styles.actionBtn, styles.rejectBtn, actionLoading && {
          opacity: 0.5
        }]} onPress={onReject} disabled={actionLoading}>
            <XSquare size={16} color="#DC2626" />
            <Text style={styles.rejectText}>{actionLoading ? 'Wait...' : 'Reject'}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.actionBtn, styles.approveBtn, actionLoading && {
          opacity: 0.5
        }]} onPress={onApprove} disabled={actionLoading}>
      ...
      <Animated.View pointerEvents={flippedDoc ? 'auto' : 'none'} style={[styles.cardFace, styles.cardBack, backAnimatedStyle]}>
        ...
        flex: 1
            ...
            <View style={styles.previewBody}>
              <Image source={{
            uri: flippedDoc.data.replace(/\s+/g, '')
...
export const PendingRegistrations = () => {
  ...
  const rejectionOptions = ['Aadhar Card', 'PAN Card', 'GST Certificate', 'Company PAN', 'Bank Cheque', 'Image is blurry/unreadable', 'Document name mismatch'];
  ...
  const fetchPending = async () => {
    try {
      ...
      error(t("Failed to load pending registrations"));
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      ...
      if (response && response.documents) {
        setLoadedDocuments((prev) => ({
          ...prev,
          [userId]: response.documents
        }));
      }
    } catch (err) {
      error(t("Failed to load documents for this customer"));
    } finally {
      setLoadingDocuments((prev) => ({
        ...prev,
        [userId]: false
  ...
  return <ScrollView style={styles.container}>
      <View style={styles.headerRow}>
        ...
        marginRight: 12
          ...
          flex: 1
            ...
            <Text style={styles.headerTitle} numberOfLines={1} adjustsFontSizeToFit>{t('Pending Registrations')}</Text>
            <Text style={styles.headerSub} numberOfLines={2}>{t('Review and approve new retailer accounts')}</Text>
          </View>
        </View>
      ...
      <View style={styles.content}>
        {pendingUsers.length === 0 ? <View style={styles.emptyBox}>
            <CheckSquare size={48} color="#D1D1D6" style={{
          marginBottom: 16
        }} />
            <Text style={styles.emptyTitle}>{t('All Caught Up!')}</Text>
            <Text style={styles.emptySub}>{t('There are no pending registrations at the moment.')}</Text>
          </View> : paginatedUsers.map((user) => <RegistrationCard key={user.UserID} user={user} userDocs={loadedDocuments[user.UserID]} isDocLoading={loadingDocuments[user.UserID]} onLoadDocs={() => loadUserDocuments(user.UserID)} onApprove={() => handleAction(user.UserID, 'approve')} onReject={() => setRejectingUser(user)} actionLoading={actionLoading === user.UserID} />)}
      </View>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>{t('Reject Registration')}</Text>
            ...
            <View style={styles.modalBody}>
              <Text style={styles.modalText}>
                {t("Please select the reason(s) for rejecting")} <Text style={{
                fontWeight: '700'
              }}>{rejectingUser?.Name}</Text>{t("'s registration. This will be shown to the customer.")}
              </Text>
              <View style={styles.reasonsList}>
                {rejectionOptions.map((r) => <TouchableOpacity key={r} style={styles.reasonRow} onPress={() => toggleReason(r)}>
            ...
            <View style={styles.modalFooter}>
              ...
              <TouchableOpacity style={[styles.confirmRejectBtn, rejectionReasons.length === 0 && {
              opacity: 0.5
            }]} disabled={rejectionReasons.length === 0 || actionLoading === rejectingUser?.UserID} onPress={() => handleAction(rejectingUser.UserID, 'reject', rejectionReasons.join(', '))}>
