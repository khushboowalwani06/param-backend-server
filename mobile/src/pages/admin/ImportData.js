...
export const ImportData = () => {
  ...
  const importOptions = [
  { id: 'customers', label: t('Import Customers'), icon: <Users size={24} color="#3B82F6" />, active: true, desc: t('Import customer details and create accounts.') },
  { id: 'rewards', label: t('Import Customer Rewards'), icon: <Gift size={24} color="#3B82F6" />, active: true, desc: t('Assign reward targets based on Cust Num.') },
  { id: 'inventory', label: t('Import Inventory'), icon: <Package size={24} color="#9CA3AF" />, active: false, desc: t('Update products and stock levels (Coming Soon)') },
  { id: 'invoices', label: t('Import Invoices'), icon: <FileText size={24} color="#9CA3AF" />, active: false, desc: t('Import generated invoices (Coming Soon)') },
  { id: 'orders', label: t('Import Orders'), icon: <ShoppingCart size={24} color="#9CA3AF" />, active: false, desc: t('Bulk import previous orders (Coming Soon)') }];
  ...
  const handleButtonClick = async (type) => {
    try {
      ...
      setLoadingType(type);
      setLoadingMessage(t("Reading and parsing Excel file..."));
      ...
      if (type === 'customers') {
        ...
        error(t("This import type is not yet fully implemented."));
      }
    } catch (err) {
      console.error(err);
      error(t("Failed to parse the Excel file."));
    } finally {
      setLoadingType(null);
      setLoadingMessage('');
    }
    ...
    return (
      <View style={styles.loadingContainer}>
        <View style={styles.loadingCard}>
          ...
          <Text style={styles.loadingTitle}>{t('Process Ongoing')}</Text>
          <Text style={styles.loadingMsg}>{loadingMessage}</Text>
          <Text style={styles.loadingWarning}>{t('Please do not close this window until the import is complete.')}</Text>
        </View>
      </View>);
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>{t('Data Import Hub')}</Text>
        <Text style={styles.headerSub}>{t('Bulk import your data using Excel spreadsheets')}</Text>
      </View>
