...
const DocumentCard = ({
  ...
  const DocumentRow = ({
    ...
    return <View style={styles.docRow}>
        ...
        <View style={styles.docActions}>
          ...
          {!isPdf && <TouchableOpacity onPress={() => onPreview({
          label,
          data: docBase64
        })} style={styles.docBtn}>
  ...
  return <View style={styles.card}>
      <View style={styles.cardHeader}>
        ...
        <View style={styles.approvedBadge}><Text style={styles.approvedText}>{t('APPROVED')}</Text></View>
      </View>

      <View style={styles.cardBody}>
        <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Email')}</Text><Text style={styles.infoVal}>{user.Email}</Text></View>
        <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Phone')}</Text><Text style={styles.infoVal}>{user.Phone || 'N/A'}</Text></View>
        <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('GSTIN')}</Text><Text style={styles.infoVal}>{user.BPID || 'N/A'}</Text></View>
      </View>
...
export const CustomerDocuments = () => {
  ...
  const fetchApproved = async () => {
    try {
      ...
      const approvedCustomers = allUsers.filter((u) => (u.Role === 'customer' || u.Role === 'retailer' || u.Role === 'dealer') && u.ApprovalStatus !== 'Pending' && u.ApprovalStatus !== 'Rejected');
  ...
  return <ScrollView style={styles.container}>
      <View style={styles.headerRow}>
        <View style={styles.headerInfo}>
          ...
          <View>
            <Text style={styles.headerTitle}>{t('Customer Documents')}</Text>
            <Text style={styles.headerSub}>{t('View submitted documents')}</Text>
          </View>
        </View>
      </View>

      <View style={styles.searchWrapper}>
        <SearchFilter value={searchTerm} onChange={setSearchTerm} placeholder={t("Search customers...")} />
      </View>

      <View style={styles.content}>
        {filteredUsers.length === 0 ? <View style={styles.emptyBox}>
          ...
          </View> : paginatedUsers.map((user) => <DocumentCard key={user.UserID} user={user} userDocs={loadedDocuments[user.UserID]} isDocLoading={loadingDocuments[user.UserID]} onLoadDocs={() => loadUserDocuments(user.UserID)} onPreview={handlePreview} />)}
      </View>
      ...
      <Modal visible={!!previewDoc} transparent animationType="slide">
        <View style={styles.previewOverlay}>
          ...
          <View style={styles.previewBody}>
            {previewDoc && <Image source={{
            uri: previewDoc.data.replace(/\s+/g, '')
          }} style={styles.previewImg} contentFit="contain" />}
          </View>
        </View>
      </Modal>
    </ScrollView>;
