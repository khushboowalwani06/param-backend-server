...
const RetailerCard = React.memo(({
  ...
  const [profileForm, setProfileForm] = useState({
    ...
    City: r.City || '',
    District: r.District || '',
    Segment: r.Segment || (r.NonTradeActivated ? 'Non-Trade' : 'Trade')
  });
  ...
  const renderFront = () => {
    ...
    return <Animated.View pointerEvents={flipped ? 'none' : 'auto'} style={[styles.cardFace, {
      ...
      <View style={styles.infoGrid}>
        ...
        <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Address')}</Text><Text style={[styles.infoValue, {
            textAlign: 'right',
            flex: 1
          }]}>{r.Address || 'N/A'}</Text></View>
        <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('PAN & GST')}</Text><Text style={styles.infoValue} numberOfLines={1}>{r.Documents?.pan || 'N/A'} | {r.Documents?.gst || 'N/A'}</Text></View>
        <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Email')}</Text><Text style={styles.infoValue} numberOfLines={1}>{r.Email || r.EmailAddress || 'No Email'}</Text></View>
        {user?.Role === 'admin' && <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Password')}</Text><Text style={styles.infoValue} selectable={true}>{r.Password || 'N/A'}</Text></View>}
        <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Sales Rep')}</Text><View style={{
            alignItems: 'flex-end'
          }}><Text style={styles.infoValue}>{repName}</Text>{repId && <Text style={{
              fontSize: 10,
              color: '#8E8E93'
            }}>{repId}</Text>}</View></View>
        <View style={styles.infoRow}><Text style={styles.infoLabel}>{t('Total Orders')}</Text><Text style={[styles.infoValue, {
            fontWeight: '700'
          }]}>{ordersCount}</Text></View>
      </View>

      <View style={styles.financeGrid}>
        <View style={styles.financeBox}>
          <Text style={styles.financeLabel}>{t('CREDIT LIMIT')}</Text>
          <View style={{
            flexDirection: 'row',
            alignItems: 'center'
          }}>
            {editingCredit ? <View style={{
              flexDirection: 'row',
              alignItems: 'center'
            }}>
                <TextInput value={editLimit} onChangeText={setEditLimit} style={styles.editInput} keyboardType="numeric" />
                <TouchableOpacity onPress={() => {
                handleSaveCreditLimit(r.UserID, editLimit);
                setEditingCredit(false);
              }} style={styles.iconBtn}><Check size={14} color="#FFF" /></TouchableOpacity>
                <TouchableOpacity onPress={() => setEditingCredit(false)} style={[styles.iconBtn, {
                backgroundColor: '#E5E5EA'
              }]}><X size={14} color="#1A1A1A" /></TouchableOpacity>
              </View> : <Text style={styles.financeValue}>{formatCurrency(creditLimit)}</Text>}
            {!editingCredit && user?.Role && ['admin', 'accountant'].includes(user.Role) && <TouchableOpacity onPress={() => setEditingCredit(true)} style={{
              marginLeft: 8
            }}>
        ...
        <View style={styles.financeBox}>
          <Text style={styles.financeLabel}>{t('SPENDABLE')}</Text>
          <Text style={[styles.financeValue, {
            color: '#34C759'
          }]}>{formatCurrency(spendable)}</Text>
        </View>
      </View>

      <View style={styles.outstandingRow}>
        <Text style={styles.financeLabel}>{t('OUTSTANDING')}</Text>
        <Text style={[styles.financeValue, {
          fontSize: 20,
          color: outstColor
        }]}>{formatCurrency(outst)}</Text>
      </View>
      ...
      <View style={styles.rewardsBox}>
        ...
        <View style={{
          ...
          <View style={styles.rewardTextRow}><Text style={styles.rewardLabel}>{t('Achieved:')}</Text><Text style={styles.rewardVal}>{achievedValue.toFixed(1)}T</Text></View>
          <View style={styles.rewardTextRow}><Text style={styles.rewardLabel}>{t('Target:')}</Text><Text style={styles.rewardVal}>{targetValue}T</Text></View>
          <View style={styles.progressBar}><View style={[styles.progressFill, {
              width: `${Math.min(100, achievedValue / targetValue * 100)}%`
            }]} /></View>
        </View>
      </View>
      ...
      {user?.Role && ['admin', 'sales', 'accountant'].includes(user.Role) && <View style={styles.actionRow}>
          {user?.Role === 'admin' && <>
              <TouchableOpacity onPress={() => onDelete(r.UserID)} style={[styles.actionBtn, {
            backgroundColor: '#FEE2E2'
          }]}>
                <Trash2 size={14} color="#DC2626" /><Text style={[styles.actionBtnText, {
              color: '#DC2626'
            }]}>{t('Delete')}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => flipTo(180, 'profile')} style={[styles.actionBtn, {
            backgroundColor: '#F9FAFB',
            borderWidth: 1,
            borderColor: '#E5E5EA'
          }]}>
                <Edit2 size={14} color="#1A1A1A" /><Text style={[styles.actionBtnText, {
              color: '#1A1A1A'
            }]}>{t('Edit Details')}</Text>
              </TouchableOpacity>
            </>}
          {user?.Role !== 'accountant' && <TouchableOpacity onPress={() => onOrder(r)} style={[styles.actionBtn, {
          backgroundColor: '#E0F2FE'
        }]}>
              <ShoppingCart size={14} color="#0284C7" /><Text style={[styles.actionBtnText, {
            color: '#0284C7'
          }]}>{t('Place Order')}</Text>
            </TouchableOpacity>}
          <TouchableOpacity onPress={() => flipTo(180, 'rewards')} style={[styles.actionBtn, {
          backgroundColor: '#F3E8FF'
        }]}>
            <Gift size={14} color="#9333EA" /><Text style={[styles.actionBtnText, {
            color: '#9333EA'
          }]}>{t('Rewards')}</Text>
          </TouchableOpacity>
        </View>}
  ...
  const renderBack = () => {
    ...
    return <Animated.View pointerEvents={flipped ? 'auto' : 'none'} style={[styles.cardFace, styles.cardFaceBack, {
      ...
      <ScrollView style={{
        ...
        {flipContext === 'rewards' ? <View style={{
          gap: 12
        }}>
            <View>
      ...
      <View style={styles.actionRowEnd}>
        <TouchableOpacity onPress={() => flipTo(0, null)} style={styles.cancelBtn}><Text style={{
            color: '#1A1A1A',
            fontWeight: '600'
          }}>{t('Cancel')}</Text></TouchableOpacity>
        <TouchableOpacity style={[styles.saveBtn, (savingProfile || savingRewards) && {
          opacity: 0.7
        }]} onPress={async () => {
          ...
          <Text style={{
            color: '#FFF',
            fontWeight: '600'
          }}>{savingProfile || savingRewards ? 'Saving...' : 'Save'}</Text>
        </TouchableOpacity>
      </View>
    </Animated.View>;
  };
...
export const RetailersDirectory = ({
  ...
  const renderRetailerItem = useCallback(({
    ...
    const agg = orderAggregates[r.UserID] || {
      count: 0,
      achievedValue: 0
    };
    const isNonTrade = segment === 'Non-Trade';
    const repId = r.AssignedSalesRep;
    return <RetailerCard key={r.UserID} r={r} user={user} repName={repId ? salesRepsMap[repId] || 'Unknown Rep' : 'Unassigned'} repId={repId} outst={outst} creditLimit={creditLimit} spendable={spendable} segment={segment} ordersCount={agg.count} outstColor={outst === 0 ? '#34C759' : '#F59E0B'} badgeBg={isNonTrade ? '#F3E8FF' : '#E0F2FE'} badgeColor={isNonTrade ? '#9333EA' : '#0284C7'} targetValue={rewardsMap[r.UserID]?.targets?.[1]?.target || 280} achievedValue={agg.achievedValue} onDelete={handleDeleteCustomer} onOrder={handleOrder} handleSaveCreditLimit={handleSaveCreditLimit} availableDistricts={availableDistricts} onSaveProfile={handleSaveProfile} onSaveRewards={handleSaveRewards} rewardsData={rewardsMap[r.UserID]} />;
      ...
      {!compactMode && <View style={styles.header}>
          ...
          <View style={styles.filterRow}>
            <View style={styles.searchBox}>
              <Search size={16} color="#8E8E93" style={{
            position: 'absolute',
            left: 12
          }} />
