...
const FlippableCard = ({ order, isSelected, toggleSelect, onOpenInvoice }) => {
  const [flipped, setFlipped] = useState(false);
  const flipAnim = useSharedValue(0);

  const flipCard = () => {
    setFlipped(!flipped);
    flipAnim.value = withTiming(flipped ? 0 : 1, { duration: 400 });
  };
    ...
    <View style={styles.cardContainer}>
      <Animated.View style={[styles.card, frontStyle]}>
        <View style={styles.cardHeader}>
          <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, paddingRight: 8 }}>
            <TouchableOpacity onPress={toggleSelect} style={{ marginRight: 8, padding: 4 }}>
        ...
        <View style={styles.cardFooter}>
          {order.InvoicePdf || order.InvoicePdfLink ? (
            <TouchableOpacity
              ...
              onPress={() => onOpenInvoice(order.InvoicePdf || order.InvoicePdfLink)}
            >
          ...
          {receiptUrl && (
            <TouchableOpacity style={styles.receiptBtn} onPress={flipCard}>
      ...
      <Animated.View style={[styles.card, styles.cardBack, backStyle]} pointerEvents={flipped ? 'auto' : 'none'}>
        <View style={styles.cardBackHeader}>
          <Text style={styles.cardBackTitle}>Payment Receipt</Text>
  ...
  const generateBulkZip = async () => {
    ...
    try {
        ...
        if (pdfUrl) {
          try {
            ...
            if (pdfUrl.startsWith('data:application/pdf;base64,')) {
              ...
              folder.file(`${order.OrdID}_Invoice.pdf`, base64Data, { base64: true });
            } else {
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(fileUri, { mimeType: "application/zip", dialogTitle: "Download Invoices ZIP" });
      } else {
    ...
    <View style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item: order }) => (
          <FlippableCard 
            order={order} 
            isSelected={selectedOrders.has(order.OrdID)}
            toggleSelect={() => toggleSelect(order.OrdID)}
...
const styles = StyleSheet.create({
  ...
  filtersContainer: { paddingHorizontal: 16, paddingTop: 16, backgroundColor: '#F8FAFC', gap: 12 },
  filterScroll: { gap: 12, paddingBottom: 4 },
  actionsBar: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12 },
  selectAllBtn: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  selectAllText: { fontSize: 14, fontWeight: '500', color: '#64748B' },
  bulkDownloadBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#3B82F6', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 8 },
  bulkDownloadText: { color: 'white', fontWeight: '600', fontSize: 14 },
  listContainer: { flex: 1, paddingHorizontal: 16 },
  emptyState: { alignItems: 'center', justifyContent: 'center', paddingVertical: 60, backgroundColor: '#FFF', borderRadius: 12, borderWidth: 1, borderColor: '#E2E8F0', marginTop: 16 },
  emptyText: { marginTop: 12, fontSize: 16, color: '#94A3B8', fontWeight: '500' },
  cardContainer: { marginBottom: 16, minHeight: 180 },
  card: { backgroundColor: '#FFF', borderRadius: 12, padding: 16, borderWidth: 1, borderColor: '#E2E8F0', ...Platform.select({ ios: { shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 4 }, android: { elevation: 2 }, web: { boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' } }) },
  cardBack: { backgroundColor: '#F8FAFC' },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 },
  ordId: { fontSize: 16, fontWeight: 'bold', color: '#0F172A' },
  dateText: { fontSize: 12, color: '#64748B', marginTop: 4 },
  badge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 999 },
  badgeText: { fontSize: 12, fontWeight: '600' },
  cardBody: { gap: 12, marginBottom: 16 },
  row: { flexDirection: 'row', alignItems: 'flex-start' },
  label: { width: 100, fontSize: 14, color: '#64748B', fontWeight: '500' },
  value: { flex: 1, fontSize: 14, color: '#1A1A1A' },
  valueBold: { flex: 1, fontSize: 14, fontWeight: 'bold', color: '#1A1A1A' },
  ...
  invoiceBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#F1F5F9', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 6 },
  invoiceText: { fontSize: 13, fontWeight: '600', color: '#1A1A1A' },
  receiptBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#F0FDF4', borderWidth: 1, borderColor: '#BBF7D0', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 6 },
  receiptText: { fontSize: 13, fontWeight: '600', color: '#16A34A' },
  cardBackHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  cardBackTitle: { fontSize: 16, fontWeight: 'bold', color: '#1A1A1A' },
  closeFlipBtn: { padding: 4, backgroundColor: '#F1F5F9', borderRadius: 20 },
  receiptImage: { width: '100%', height: 120, borderRadius: 8 },
  noReceiptContainer: { height: 120, justifyContent: 'center', alignItems: 'center', backgroundColor: '#F1F5F9', borderRadius: 8 },
  noReceiptText: { color: '#64748B', fontSize: 14 }
});
