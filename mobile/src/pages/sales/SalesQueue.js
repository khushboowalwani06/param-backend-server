...
const QueueCard = ({
  ...
  return <View style={styles.card}>
      ...
      <View style={styles.headline}>
        ...
        {isAging && <Text style={styles.agingText}>{t('Waiting >24h')}</Text>}
      </View>
      ...
      <View style={styles.grid}>
        ...
        {order.City ? <View style={[styles.gridItem, {
        width: '100%',
        marginTop: 8
      }]}>
        ...
        marginTop: 8
          ...
          marginTop: 4
            ...
            <TextInput style={styles.priceInput} value={editedPrice} onChangeText={setEditedPrice} keyboardType="numeric" />
            {editedPrice !== order.EstimateAmt?.toString() && <TouchableOpacity style={styles.savePriceBtn} onPress={handleSavePrice} disabled={isSavingPrice}>
                {isSavingPrice ? <ActivityIndicator size="small" color="#FFF" /> : <Save size={14} color="#FFF" />}
                <Text style={styles.savePriceText}>{t('SAVE')}</Text>
              </TouchableOpacity>}
          </View>
        </View>
      </View>

      {order.Notes || hasAudio ? <View style={styles.notesBox}>
          <Text style={styles.sectionLabel}>{t('NOTES')}</Text>
          {order.Notes && order.Notes.replace(' [Audio Note Attached]', '').trim() ? <Text style={styles.notesText}>"{order.Notes.replace(' [Audio Note Attached]', '')}"</Text> : null}
          {hasAudio && <View style={{
        marginTop: 8
      }}>
      ...
      {isExpanded && hist && <View style={styles.histGrid}>
  ...
  return <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{t('Sales Queue')}</Text>
          <Text style={styles.subtitle}>{orders.length} Orders awaiting sales review</Text>
        </View>

        <View style={styles.targetsCard}>
          <Text style={styles.targetsTitle}>{t('My Monthly Targets')}</Text>
          <View style={styles.targetsGrid}>
            <View style={styles.targetCol}>
              <Text style={styles.targetLabel}>{t('Volume (Tons)')}</Text>
              <Text style={styles.targetVal} adjustsFontSizeToFit numberOfLines={1}>{monthlyVolume.toFixed(1)} / {VOLUME_TARGET}</Text>
              <View style={styles.progressBar}>
                <View style={[styles.progressFill, {
                width: `${Math.min(100, monthlyVolume / VOLUME_TARGET * 100)}%`
              }]} />
              </View>
            </View>
            <View style={styles.targetCol}>
              <Text style={styles.targetLabel}>{t('Daily Visits')}</Text>
              <Text style={styles.targetVal}>{dailyVisits} / {VISITS_TARGET}</Text>
              <View style={styles.progressBar}>
                <View style={[styles.progressFill, {
                width: `${Math.min(100, dailyVisits / VISITS_TARGET * 100)}%`
              }]} />
              </View>
            </View>
          </View>
        ...
        <View style={styles.viewToggles}>
          <TouchableOpacity style={[styles.toggleBtn, viewMode === 'queue' && styles.toggleBtnActive]} onPress={() => {
          setViewMode('queue');
          setCurrentPage(1);
          setSearchQuery('');
        }}>
            <Text style={[styles.toggleBtnText, viewMode === 'queue' && styles.toggleBtnTextActive]}>{t('Pending Approvals')}</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.toggleBtn, viewMode === 'balances' && styles.toggleBtnActive]} onPress={() => {
          setViewMode('balances');
          setCurrentPage(1);
          setSearchQuery('');
        }}>
            <Text style={[styles.toggleBtnText, viewMode === 'balances' && styles.toggleBtnTextActive]}>{t('Outstanding Balances')}</Text>
          </TouchableOpacity>
        </View>
        ...
        {viewMode === 'queue' ? <>
            ...
            {filteredOrders.length === 0 ? <View style={styles.emptyState}>
                ...
                <Text style={styles.emptySub}>{t('No orders currently awaiting your approval.')}</Text>
              </View> : paginatedOrders.map((order, idx) => <QueueCard key={order.OrdID} order={order} index={idx} isExpanded={expandedOrders[order.OrdID]} onToggleExpand={() => handleToggleExpand(order)} hist={customerHistory[order.OrdID]} isSubmitting={submittingIds.has(order.OrdID)} onApprove={() => handleApprove(order.OrdID)} onReject={() => setRejectingOrder(order.OrdID)} onEdit={() => setEditingOrder(order)} onSavePrice={handleSavePrice} />)}
          </> : <>
            <View style={styles.tools}>
              <View style={{
            flex: 1,
            ...
            <View style={styles.dealersList}>
              <View style={styles.dealerHeaderRow}>
                <Text style={[styles.dealerColText, {
              flex: 2,
              fontWeight: '700'
            }]}>{t('Dealer')}</Text>
                <Text style={[styles.dealerColText, {
            ...
            return <View key={dealer.UserID} style={[styles.dealerRow, isOver && styles.dealerRowOverLimit]}>
                    <View style={{
                flex: 2,
                paddingRight: 4
              }}>
                ...
                flex: 1,
                textAlign: 'right'
              }]} adjustsFontSizeToFit numberOfLines={1}>₹{limit.toLocaleString('en-IN')}</Text>
                    <Text style={[styles.dealerColText, {
                flex: 1.4,
                fontWeight: '700',
                textAlign: 'right'
      ...
      {(viewMode === 'queue' ? totalPages : dealersTotalPages) > 1 && <Pagination currentPage={currentPage} totalPages={viewMode === 'queue' ? totalPages : dealersTotalPages} onPageChange={setCurrentPage} />}
      ...
      <Modal visible={!!rejectingOrder} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <TextInput style={styles.modalInput} placeholder="Reason for rejection..." value={rejectReason} onChangeText={setRejectReason} multiline numberOfLines={3} />
            <View style={styles.modalActions}>
              ...
              <TouchableOpacity style={styles.modalSubmit} onPress={handleRejectSubmit}>
                <Text style={styles.modalSubmitText}>{t('CONFIRM REJECT')}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
