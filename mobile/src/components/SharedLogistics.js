...
const StatusBadge = ({ status }) => {
  let color = '#64748B';
  let bgColor = '#F1F5F9';
  if (status === 'Delivered') {color = '#10B981';bgColor = '#D1FAE5';} else
  if (status === 'In Transit') {color = '#F59E0B';bgColor = '#FEF3C7';} else
  if (status === 'Dispatched') {color = '#3B82F6';bgColor = '#DBEAFE';} else
  if (status === 'Ready for Dispatch') {color = '#8B5CF6';bgColor = '#EDE9FE';}
